...
### 重要约束
**⚠️ 关键规则**: 当涉及 Unity 中的操作时，**严禁通过创建服务端脚本、测试脚本或任何 .js/.mjs 文件的方式去解决问题**。

**核心原则**:
1. **纯 MCP 工具导向**: 所有 Unity 操作必须通过现有的 MCP tools 完成，包括但不限于：
   - `create_ui_element` - 创建 UI 元素
   - `update_gameobject` - 更新游戏对象
   - `validate_ui_components` - 验证和修复 UI 组件
   - `apply_ui_style` - 应用 UI 样式
   - `get_gameobject` - 获取游戏对象信息

2. **禁止行为**:
   - ❌ 创建任何 `.js`、`.mjs` 或其他脚本文件进行 Unity 操作
   - ❌ 通过 WebSocket 客户端脚本间接操作 Unity
   - ❌ 创建测试脚本来验证或优化功能
   - ❌ 通过文件系统操作来修改 Unity 项目

3. **正确做法**:
   - ✅ 直接调用 Unity MCP Bridge 的现有工具
   - ✅ 使用工具链式调用完成复杂操作
   - ✅ 通过 MCP 工具的参数配置实现定制化
   - ✅ 依赖工具的返回数据进行状态判断和下一步操作

**强化要求**: 作为 MCP 工具的 Claude，你的价值在于智能地使用和组合现有工具，而不是创建新的脚本或工具。用户期望的是一个能够通过自然语言直接操控 Unity Editor 的智能助手，而不是一个脚本生成器。
